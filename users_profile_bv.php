...
$users_profile_blobview = NULL; // Initialize page object first

class cusers_profile_blobview extends cusers {
	...
	// Page object name
	var $PageObjName = 'users_profile_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'users');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('users', 'x_profile');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('profile');
...
// Create page object
if (!isset($users_profile_blobview)) $users_profile_blobview = new cusers_profile_blobview();

// Page init
$users_profile_blobview->Page_Init();

// Page main
$users_profile_blobview->Page_Main();
?>
<?php
$users_profile_blobview->Page_Terminate();
?>
